...
```bash
joshgann-site$ sam build --use-container
```
...
```bash
joshgann-site$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
joshgann-site$ sam local start-api
joshgann-site$ curl http://localhost:3000/
```
...
```bash
joshgann-site$ sam logs -n HelloWorldFunction --stack-name joshgann-site --tail
```
...
```bash
joshgann-site$ pip install -r tests/requirements.txt --user
# unit test
...
```bash
aws cloudformation delete-stack --stack-name joshgann-site
```
